...
const UserNavigation = () => {
    const linkClass = 'text-gray-700 hover:text-black hover:bg-gray-200 p-3 px-4 block opacity-75';
    const { authUser: { username }, setAuthUser } = useContext(AuthContext);
    ...
    return (
        ...
        <AnimationWraper key={"UserNavigation"} transition={{ duration: 0.2 }} initial={{y:10}}>
            <div className='flex flex-col absolute right-0 w-60 border border-gray-200 bg-white rounded-md'>
                ...
                <Link to={`/user/${username}`} className={linkClass}>Profile</Link>
                <Link className={linkClass}>Dashboard</Link>
                <Link className={linkClass}>Settings</Link>
                <hr />
                <Link to="/login" className={linkClass + " flex items-center gap-3"} onClick={() => {
                    logout();
